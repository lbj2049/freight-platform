...
export default {
  ...
  methods: {
    ...
    handleCloseTag (res, type, route) {
      if (type === 'all') {
        this.turnToPage(this.$config.freightHomeName)
      } else if (routeEqual(this.$route, route)) {
        if (type !== 'others') {
          const nextRoute = getNextRoute(this.tagNavList, route, 'freight')
  ...
  mounted () {
    ...
    // 如果当前打开页面不在标签栏中，跳到freightHomeName页
    if (!this.tagNavList.find(item => item.name === this.$route.name)) {
      this.$router.push({
        name: this.$config.freightHomeName
      })
    }
    // 获取未读消息条数
